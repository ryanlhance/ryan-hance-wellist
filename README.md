...
A single, shareable web page that shows the **Wellist job description exactly as posted** — and lets a reader hover any highlighted phrase (it visibly becomes clickable) and click it to see the **specific, real experience** that backs it.
...
- The real Wellist posting: intro, **What You'll Own**, **What You Bring** — styled to feel like the live job page (Wellist logo, colors, and typeface).
...
- A **shareable deep link:** click a phrase and the address bar updates (e.g. `…/#p-systems-scale`); sending that link opens the page with that phrase already expanded.
...
1. **Create a repo and push these files to its root:**
   ...
   cd wellist-brainmap
   git init
   git add index.html styles.css app.js data.json wellist-logo.svg README.md build_data.py
   git commit -m "Wellist fit map"
   git branch -M main
...
3. **Wait ~1 minute**, then open the URL Pages gives you:
   ...
   ```
   That's the link you send. Deep links work too, e.g. `…/#p-primary-lead`.
...
wellist-brainmap/
  ...
  styles.css         # Wellist brand styling + responsive
  ...
  wellist-logo.svg   # Wellist logo (recolored to brand blue)
...
The twelve portfolio pieces now link to their public **hance.work** case-study pages. To change one, edit that evidence item's `link` in `data.json` (or `build_data.py`).
